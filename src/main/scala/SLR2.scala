...
// for the PGs placed in SLR2

class SLR2_IO (implicit val conf : HBMGraphConfiguration) extends Bundle{
    ...
    // independent signal
    val end = Output(Vec(conf.slr2_channel_num, Bool()))
    val p2_count = Output(Vec(conf.slr2_channel_num, UInt(conf.Data_width.W)))
    val p2_pull_count = Output(Vec(conf.slr2_channel_num, UInt(conf.Data_width.W)))
    val frontier_pull_count = Output(Vec(conf.slr2_channel_num, UInt(conf.Data_width.W)))
    val last_iteration = Output(Vec(conf.slr2_channel_num, Bool()))
    val init_bram = Output(Vec(conf.slr2_channel_num, Bool()))

    val p2_in = Vec(conf.pipe_num_per_channel * conf.slr2_channel_num, Flipped(Decoupled(UInt((conf.Data_width*2).W))))
    val frontier_in = Vec(conf.pipe_num_per_channel * conf.slr2_channel_num, Flipped(Decoupled(UInt(conf.Data_width.W))))
    val p2_out = Vec(conf.pipe_num_per_channel * conf.slr2_channel_num, Decoupled(UInt(conf.Data_width.W)))

    val R_array_index = Vec(conf.pipe_num_per_channel * conf.slr2_channel_num, Decoupled(UInt(conf.Data_width.W)))
    val p1_end = Output(Vec(conf.slr2_channel_num, Bool()))
    val uram_out_a = Vec(conf.pipe_num_per_channel * conf.slr2_channel_num, Output(UInt(conf.Data_width_uram.W)))
    val uram_addr_a = Input(Vec(conf.pipe_num_per_channel * conf.slr2_channel_num, UInt(conf.Addr_width_uram.W)))

}


class SLR2 (implicit val conf : HBMGraphConfiguration) extends Module{
	val io = IO(new SLR2_IO())
	dontTouch(io)

    val reset_reg = RegNext(reset)
    val reset_reg_array = VecInit(Seq.fill(conf.slr2_channel_num)(RegNext(reset))) //new Array[Reset](conf.channel_num)

    val pipeline_array = new Array[pipeline](conf.slr2_channel_num)
    for(i <- 0 until conf.slr2_channel_num) {
        pipeline_array(i) = withReset(reset_reg_array(i)){Module(new pipeline(i + conf.channel_num - conf.slr2_channel_num))}
    }

    val copy_start = Module(new copy_slr2(1))
    val copy_frontier_flag = Module(new copy_slr2(1))
    val copy_current_level = Module(new copy_slr2(32))
    val copy_p2_end = Module(new copy_slr2(1))
    val copy_push_or_pull = Module(new copy_slr2(1))
    val copy_node_num = Module(new copy_slr2(conf.Data_width))
    val copy_root = Module(new copy_slr2(conf.Data_width))
    ...
    val copy_offset_CSR_R_offset = Module(new copy_slr2(conf.Data_width))
    val copy_offset_CSR_C_offset = Module(new copy_slr2(conf.Data_width))
    val copy_offset_CSC_R_offset = Module(new copy_slr2(conf.Data_width))
    val copy_offset_CSC_C_offset = Module(new copy_slr2(conf.Data_width))
    val copy_offset_level_offset = Module(new copy_slr2(conf.Data_width))
    ...
    val r_array_count = RegInit(VecInit(Seq.fill(conf.slr2_channel_num * conf.pipe_num_per_channel)(0.U(conf.Data_width.W))))
